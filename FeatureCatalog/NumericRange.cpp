...
void NumericRange::GetContents(pugi::xml_node& node)
{
	for (pugi::xml_node instruction = node.first_child(); instruction; instruction = instruction.next_sibling())
	{
		...
		if (!strcmp(instructionName, "S100Base:lowerBound"))
		{
			...
			////KRS_MSG_PROCESS::SendMessageToTargetWindow(KRS_MSG_INFO, L"S100Base:lowerBound setting Clear", ENCCommon::Developer_Mode, ENCCommon::FC);
		}

		else if (!strcmp(instructionName, "S100Base:upperBound"))
		{
			upperBound = std::stod(instruction.child_value());
			////KRS_MSG_PROCESS::SendMessageToTargetWindow(KRS_MSG_INFO, L"S100Base:upperBound setting Clear", ENCCommon::Developer_Mode, ENCCommon::FC);
		}

		else if (!strcmp(instructionName, "S100Base:intervalType"))
		{
			intervalType.GetContents(instruction);
			////KRS_MSG_PROCESS::SendMessageToTargetWindow(KRS_MSG_INFO, L"S100Base:intervalType setting Clear", ENCCommon::Developer_Mode, ENCCommon::FC);
		}
	}
}
